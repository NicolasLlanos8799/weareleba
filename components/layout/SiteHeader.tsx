...
const navigation = {
  en: [["Portfolio", "/workinprocess"], ["Solutions", "/#what-we-do"], ["About", "/about"]],
  es: [["Portfolio", "/workinprocess"], ["Soluciones", "/#what-we-do"], ["Nosotros", "/nosotros"]],
};
...
export function SiteHeader({ tone = "light" }: SiteHeaderProps) {
  ...
  useEffect(() => {
    ...
    const updateContrast = () => {
      ...
      const samplePoints = [[window.innerWidth * 0.12, headerHeight], [window.innerWidth * 0.5, headerHeight], [window.innerWidth * 0.88, headerHeight]];
      ...
      for (const [x, y] of samplePoints) {
        ...
        while (current && current !== document.body) {
          ...
          if (computed.backgroundColor !== "rgba(0, 0, 0, 0)" && computed.backgroundColor !== "transparent") { background = computed.backgroundColor; break; }
        ...
        if (match) {
          ...
          const [r, g, b] = parts;
          if ([r, g, b].every((value) => Number.isFinite(value))) luminances.push((0.2126 * r + 0.7152 * g + 0.0722 * b) / 255);
        }
      }
      if (luminances.length) setIsDarkBackground(luminances.reduce((sum, value) => sum + value, 0) / luminances.length < 0.52);
    };
    const onScroll = () => { cancelAnimationFrame(frame); frame = requestAnimationFrame(updateContrast); };
    updateContrast();
    window.addEventListener("scroll", onScroll, { passive: true });
    window.addEventListener("resize", updateContrast);
    return () => { cancelAnimationFrame(frame); window.removeEventListener("scroll", onScroll); window.removeEventListener("resize", updateContrast); };
  }, []);
  ...
  const headerStyle = { background: "transparent", backdropFilter: "blur(28px) saturate(115%)", WebkitBackdropFilter: "blur(28px) saturate(115%)", position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, color: foreground, "--header-fg": foreground, "--header-muted": muted, "--header-border": border, transition: "color 220ms ease" } as CSSProperties;
