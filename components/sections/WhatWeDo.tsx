...
function ServiceVisual({ type, language }: { type: string; language: "en" | "es" }) {
  ...
  const websiteMotion = { "--parallax-speed": ".18" } as CSSProperties;
  const softwareMotion = { "--parallax-speed": ".12" } as CSSProperties;
  const automationMotion = { "--parallax-speed": ".16" } as CSSProperties;

  if (type === "website") return <div className="service-visual service-visual-website" aria-hidden="true" data-parallax style={websiteMotion}><div className="visual-laptop"><div className="visual-laptop-screen"><div className="visual-browser-bar"><span /><span /><span /></div><div className="visual-website-copy"><small>WE ARE LEBA</small><strong>{isSpanish ? <>Tu negocio,<br />en su mejor versión.</> : <>Your business,<br />at its best.</>}</strong><i /></div><div className="visual-website-orb" /></div><div className="visual-laptop-base" /></div></div>;
  if (type === "software") return <div className="service-visual service-visual-software" aria-hidden="true" data-parallax style={softwareMotion}><div className="visual-dashboard"><aside><b>{isSpanish ? "Tu Negocio" : "Your Business"}</b><span>{isSpanish ? "Inicio" : "Home"}</span><span>{isSpanish ? "Clientes" : "Clients"}</span><span>{isSpanish ? "Servicios" : "Services"}</span><span>{isSpanish ? "Reservas" : "Bookings"}</span><span>{isSpanish ? "Reportes" : "Reports"}</span></aside><div className="visual-dashboard-main"><div className="visual-dashboard-title">{isSpanish ? "Resumen" : "Overview"}</div><div className="visual-stat-grid"><div><small>{isSpanish ? "Clientes" : "Clients"}</small><b>124</b><em>↗ 12%</em></div><div><small>{isSpanish ? "Reservas" : "Bookings"}</small><b>58</b><em>↗ 8%</em></div></div><div className="visual-list-title">{isSpanish ? "Próximas reservas" : "Upcoming bookings"}</div><div className="visual-list-row"><i /> <span>Maria Gonzalez</span><small>10:00</small></div><div className="visual-list-row"><i /> <span>Juan Perez</span><small>11:30</small></div></div></div></div>;
  return <div className="service-visual service-visual-automation" aria-hidden="true" data-parallax style={automationMotion}><div className="automation-flow"><div className="automation-inputs"><div>↗ <span>{isSpanish ? "Nuevos mensajes" : "New messages"}</span></div><div>▣ <span>{isSpanish ? "Nuevas reservas" : "New bookings"}</span></div><div>✉ <span>{isSpanish ? "Formularios" : "Forms"}</span></div><div>≋ <span>{isSpanish ? "Otras herramientas" : "Other tools"}</span></div></div><div className="automation-core">✦<small>{isSpanish ? "Automatizar" : "Automate"}</small></div><div className="automation-output"><span>✓ &nbsp; {isSpanish ? "Responder" : "Reply"}</span><span>✓ &nbsp; {isSpanish ? "Crear tarea" : "Create task"}</span><span>✓ &nbsp; {isSpanish ? "Actualizar cliente" : "Update client"}</span><span>✓ &nbsp; {isSpanish ? "Enviar notificación" : "Send notification"}</span></div></div></div>;
}

export function WhatWeDo() {
  const { language } = useLanguage();
  const copy = content[language];
  return <section className="what-we-do" id="what-we-do"><div className="what-we-do-inner"><div className="what-we-do-intro"><div className="section-kicker" data-reveal="left"><span /> WHAT WE DO</div><h2 data-reveal>{copy.heading}</h2><p className="motion-stagger-1" data-reveal="right">{copy.intro}</p></div><div className="service-grid">{copy.services.map((service, index) => <article className={`service-card motion-stagger-${index + 1}`} data-reveal="scale" key={service.title}><div className="service-topline"><span>{service.number}</span><i /></div><ServiceVisual type={service.visual} language={language} /><h3>{service.title}</h3><p className="service-description">{service.description}</p><p className="service-note">{service.note}</p><a className="service-link focusable" href="#contact">{copy.learnMore} <span /></a></article>)}</div><div className="what-we-do-footer" data-reveal><span><i /> {copy.footer}</span><span>WE ARE LEBA</span></div></div></section>;
}
