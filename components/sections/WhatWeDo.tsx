...
function ServiceVisual({ type, language }: { type: string; language: "en" | "es" }) {
  ...
  if (type === "website") return <div className="service-visual service-visual-website" aria-hidden="true" data-parallax style={{ "--parallax-speed": ".18" } as React.CSSProperties}><div className="visual-laptop"><div className="visual-laptop-screen"><div className="visual-browser-bar"><span /><span /><span /></div><div className="visual-website-copy"><small>WE ARE LEBA</small><strong>{isSpanish ? <>Tu negocio,<br />en su mejor versión.</> : <>Your business,<br />at its best.</>}</strong><i /></div><div className="visual-website-orb" /></div><div className="visual-laptop-base" /></div></div>;
  if (type === "software") return <div className="service-visual service-visual-software" aria-hidden="true" data-parallax style={{ "--parallax-speed": ".12" } as React.CSSProperties}><div className="visual-dashboard"><aside><b>{isSpanish ? "Tu Negocio" : "Your Business"}</b><span>{isSpanish ? "Inicio" : "Home"}</span><span>{isSpanish ? "Clientes" : "Clients"}</span><span>{isSpanish ? "Servicios" : "Services"}</span><span>{isSpanish ? "Reservas" : "Bookings"}</span><span>{isSpanish ? "Reportes" : "Reports"}</span></aside><div className="visual-dashboard-main"><div className="visual-dashboard-title">{isSpanish ? "Resumen" : "Overview"}</div><div className="visual-stat-grid"><div><small>{isSpanish ? "Clientes" : "Clients"}</small><b>124</b><em>↗ 12%</em></div><div><small>{isSpanish ? "Reservas" : "Bookings"}</small><b>58</b><em>↗ 8%</em></div></div><div className="visual-list-title">{isSpanish ? "Próximas reservas" : "Upcoming bookings"}</div><div className="visual-list-row"><i /> <span>Maria Gonzalez</span><small>10:00</small></div><div className="visual-list-row"><i /> <span>Juan Perez</span><small>11:30</small></div></div></div></div>;
  return <div className="service-visual service-visual-automation" aria-hidden="true" data-parallax style={{ "--parallax-speed": ".16" } as React.CSSProperties}><div className="automation-flow"><div className="automation-inputs"><div>↗ <span>{isSpanish ? "Nuevos mensajes" : "New messages"}</span></div><div>▣ <span>{isSpanish ? "Nuevas reservas" : "New bookings"}</span></div><div>✉ <span>{isSpanish ? "Formularios" : "Forms"}</span></div><div>≋ <span>{isSpanish ? "Otras herramientas" : "Other tools"}</span></div></div><div className="automation-core">✦<small>{isSpanish ? "Automatizar" : "Automate"}</small></div><div className="automation-output"><span>✓ &nbsp; {isSpanish ? "Responder" : "Reply"}</span><span>✓ &nbsp; {isSpanish ? "Crear tarea" : "Create task"}</span><span>✓ &nbsp; {isSpanish ? "Actualizar cliente" : "Update client"}</span><span>✓ &nbsp; {isSpanish ? "Enviar notificación" : "Send notification"}</span></div></div></div>;
}
